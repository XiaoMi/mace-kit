...
for serialno in ${array[@]}; do
    ...
    adb -s $serialno push data /data/local/tmp/macekit/
    adb -s $serialno push ${ANDROID_NDK_HOME}/sources/cxx-stl/llvm-libc++/libs/armeabi-v7a/libc++_shared.so /data/local/tmp/macekit/lib/
